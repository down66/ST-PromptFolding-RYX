...
已经折叠好的文件夹可以直接拖动文件夹标题来整体移动；拖完后扩展会按新的 DOM 顺序同步 SillyTavern 的 prompt 顺序，再重新折叠一次。
...
- 文件夹标题右侧数字显示“启用子条目/全部子条目”。
- 面板只保留整理、展开、收起、启用、清空文件夹这些核心功能。
- “清空文件夹”会移除当前预设的全部文件夹标记，让列表恢复为普通条目。
